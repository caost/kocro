...
아래 항목은 서명된 앱과 TCC 권한, 실제 키보드 입력이 필요한 검증이다. TextEdit 텍스트 반영은 아래 `유니코드 게시 modifier 병합 수정`에서 실측해 통과로 판정했고, 나머지 항목은 수행하지 않아 통과 또는 실패로 판정하지 않았다.

| 대상·조건 | 실제 결과 | 판정 |
| --- | --- | --- |
| TextEdit, 한글·영문·여러 줄·이모지·조합 문자 | 게시된 텍스트가 설정과 동일(2026-09-04, ⌃⌥⌘9 트리거로 재현) | 통과 |
